...
macro_rules! read_write_struct_register {
    ($( #[doc=$text:literal] )* $read_method_name:ident, $register_type:ident, $port:expr $(,)?) => {
        $(
            #[doc=$text]
        )*
        pub fn $read_method_name(&mut self) -> ReadWrite<'_, T, $register_type> {
            fn read_function<T: PortIo, U: Register>(registers: &mut RegisterHandler<T>) -> (&mut RegisterHandler<T>, U) {
                let data = registers.0.read($port);
                let value = U::from_register_value(data);
                (registers, value)
            }

            fn write_function<T: PortIo, U: Register>(register: &mut ReadWrite<'_, T, U>) {
                register.registers.0.write($port, register.value());
            }

            ReadWrite::new(self, read_function, write_function)
        }
    };
}
...
impl <T: PortIo> RegisterHandler<T> {
    ...
    pub fn io_select_address_enabled(&mut self) -> bool {
        self.miscellaneous_output().flags().contains(MiscellaneousOutputRegisterFlags::IO_ADDRESS_SELECT)
    }

    read_write_struct_register!(
